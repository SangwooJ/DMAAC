...
class AttentionCritic(nn.Module):
    ...
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=True, attend_heads=1):
        ...
        #added for cosdis
        self.cosdis = nn.Linear(8 * hidden_dim, hidden_dim)
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        cos_dis_list = []
        for i in range(8): #num of agen
            cos_dis = []
            for j in range(8): # cos_dis of i and js
        ...
        for i in range(8):
            cos_dis_layer_in = torch.cat((cos_dis_list[i][0] * sa_encodings[0], cos_dis_list[i][1] * sa_encodings[1], cos_dis_list[i][2] * sa_encodings[2],
                cos_dis_list[i][3] * sa_encodings[3], cos_dis_list[i][4] * sa_encodings[4], cos_dis_list[i][5] * sa_encodings[5],
                cos_dis_list[i][6] * sa_encodings[6], cos_dis_list[i][7] * sa_encodings[7]), dim=1)
            cos_dis_layer_in_all.append(cos_dis_layer_in)
        # print(cos_dis_layer_in_all[0].size()) 
        cos_dis_layer_out_all = []
        for i in range(8):
            cos_dis_layer_out_all.append(self.cosdis(cos_dis_layer_in)) #out 1024*128
